...
import com.neom108.SmartHire.services.JobSeekerApplyService;
import com.neom108.SmartHire.services.JobSeekerSaveService;
...
public class JobPostActivityController {
    ...
    @GetMapping("/dashboard/") // Note - add / after dashboard
...
//            check if current user logged in is a recruiter
            if (authentication.getAuthorities().contains(new SimpleGrantedAuthority("Recruiter"))){
                List<RecruiterJobsDto> recruiterJobs = jobPostActivityService.getRecruiterJobs(((RecruiterProfile)currentUserProfile).getUserAccountId());
                model.addAttribute("jobPost",recruiterJobs);
            } else{
                // if its job seeker then display applied and saved jobs
                ...
                //check if the job is applied or saved in the past;
